...
- [x] Day 4：项目资料工具实现
- [x] Day 5：历史事件查询工具实现
- [x] Day 6：agent 工具路由联调
- [x] Day 7：验收与复盘

### 本周状态

- 当前进行中：`Week 3 已完成，进入下周规划`
- 当前阻塞项：`无`
- 下一验收点：`Week 4 告警输出能力最小闭环`
...
- 计划：进入 Step 5，完成 agent 工具路由与多问题联调。
- 实际：已新增规则路由 agent（`tool_router`），支持从自然语言问题中抽取 `symbol/exchange/days/event_type` 并自动规划 1-3 个工具调用；新增 CLI 命令 `tool-query` 用于端到端执行。新增测试 `tests/test_tool_router.py`，覆盖 5 条不同类型问题（单工具、双工具、多工具）并校验输出包含工具来源与关键证据；与现有测试合并回归共 37 个用例通过。
- 风险：当前路由策略以规则为主，面对更复杂歧义问题时可能选错工具；后续可加入 LLM 辅助规划并保留规则兜底。
- 明日：完成 Day 7 验收与复盘，补充失败案例和下周改进项。

### Day 7 记录

- 计划：跑端到端 demo，记录失败案例与下周改进项，完成 Week 3 收尾。
- 实际：已完成 Step 1-5 全部验收；已执行主链路回归命令（`ingest-all -> normalize-events -> deduplicate-events -> analyze-events -> publish-report`）并全部成功；新增 Step5 端到端演示记录 `reports/week3-step5-tool-routing-demo-2026-05-17.md`。
- 风险：
  - 失败案例 1（软失败）：`ingest-all --limit 5` 返回 `inserted=0/skipped=5`，说明重复数据较多，增量抓取窗口与去重策略需要进一步优化。
  - 失败案例 2（能力边界）：当前工具路由主要依赖规则，复杂歧义问题可能出现工具选择偏差。
- 明日：
  - Week 4 优先完成告警输出能力最小闭环（触发条件 + 输出通道占位）。
  - 为工具路由增加 LLM 规划试验入口，并保留规则兜底。
  - 为历史查询增加性能优化方案评估（JSON 索引或资产关联表）。
